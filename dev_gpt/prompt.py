...
class DevGPTPrompt(BaseChatPromptTemplate, BaseModel):
    ...
    def construct_full_prompt(self, goals: List[str]) -> str:
        prompt_start = (
            ...
            "each responsible for a specific UI functionality.\n"
            "Design components (in src/components directory) in such a way that they \n"
            "have a single responsibility and they do it well.\n"
            "Keep the data flow unidirectional by passing \n"
            "data and callbacks to child components via props.\n"
            "Use functional components and leverage hooks like 'useState', 'useEffect', and 'useContext' "
            "to manage state, perform side effects, and share data respectively.\n"
            "Avoid mutating state directly: instead use 'setState' or the 'useState' hook.\n"
            "Follow Test Driven Development method strictly. Start by writing the test, \n"
            "then run the test, write the code, then run the test again, refactor the code \n"
            "and repeat till the tests pass.\n"
            ...
            "Add comments to code and document it properly.\n"
            "And finally, don't forget to include the main components in App.js/App.tsx.\n"
